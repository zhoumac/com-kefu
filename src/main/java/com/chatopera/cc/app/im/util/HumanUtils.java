...
//	private static String payAutoAnswerCustomerServiceName;
	...
	public static void createTextMessage(ChatMessage data, String userid) {
		if (StringUtils.equalsIgnoreCase(data.getMsgtype(), MainContext.MediaTypeEnum.IMAGE.toString())) {
			createMessage(data, MainContext.MediaTypeEnum.IMAGE.toString(), userid);
		} else {
	...
	protected static void createMessage(ChatMessage data, String msgtype, String userid) {
		...
		//////////----->支付相关自动回复    by Wayne on 2019/9/3 19:14   start--->
		if (data.getCalltype() == null) {
			outMessage.setCalltype(MainContext.CallTypeEnum.IN.toString());
		} else {
			outMessage.setCalltype(data.getCalltype());
		}
		//////////<-----支付相关自动回复    by Wayne on 2019/9/3 19:14    <---end
		...
		if (agentUser == null) {
			...
			//////////----->假如是自动回复请求则不修改传递过来的名字    by Wayne on 2019/9/3 19:35   start--->
//			if (!StringUtils.equals(payAutoAnswerCustomerServiceName, data.getUsername())) {
//				data.setUsername(agentUser.getUsername());
//			}
			//////////<-----假如是自动回复请求则不修改传递过来的名字    by Wayne on 2019/9/3 19:35    <---end
...
//	public void setPayAutoAnswerCustomerServiceName(String payAutoAnswerCustomerServiceName) {
//		HumanUtils.payAutoAnswerCustomerServiceName = payAutoAnswerCustomerServiceName;
//	}
