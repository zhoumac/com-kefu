...
public class OnlineUserUtils {
    ...
    public static CousultInvite cousult(String id, ConsultInviteRepository consultRes) {
        CousultInvite consultInvite = (CousultInvite) CacheHelper.getSystemCacheBean().getCacheObject(id);
        if (consultInvite == null) {
            logger.info("CousultInvite {}", id);
            if (consultRes == null) {
                logger.info("NULL consultRes!!!!");
            ...
            if (consultInviteList.size() > 0) {
                consultInvite = consultInviteList.get(0);
                CacheHelper.getSystemCacheBean().put(id, consultInvite, null);
            }
        }
        return consultInvite;
    }
